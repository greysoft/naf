...
class ResolverConfig
{
	...
	private static final int ALTPORT_DNS = SysProps.get("greynaf.dns.altport", PacketDNS.INETPORT);
	...
	public ResolverConfig(XmlConfig cfg, com.grey.logging.Logger logger)
		throws javax.naming.NamingException, java.net.UnknownHostException
	{
		...
		if (recursive) {
			if (dns_localservers == null) throw new NAFConfigException(LOGLBL+"Recursive mode requires recursive DNS servers to be available");
		} else {
			if (auto_roots) {
				if (dns_localservers == null) throw new NAFConfigException(LOGLBL+"Auto-rootservers requires recursive DNS servers to be available");
			} else {
				if (pthnam_roots == null) throw new NAFConfigException(LOGLBL+"Must specify roots file, if auto-bootstrap is disabled");
			}
		}
		...
		if (dns_interceptor != null || dns_port != PacketDNS.INETPORT) {
	...
	private int getLowater(XmlConfig cfg, String xpath, int hiwater)
	{
