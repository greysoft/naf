...
public class CacheManagerTest
{
	...
	public void testRootsManual() throws Exception
	{
		...
		org.junit.Assert.assertEquals(Packet.QCLASS_INET, rr.rrClass());
	...
	public void testRootsAuto() throws Exception
	{
		...
		org.junit.Assert.assertEquals(Packet.INETPORT, tsap1.getPort());
		...
		org.junit.Assert.assertEquals(Packet.INETPORT, tsap.getPort());
	...
	public void testCacheSingular() throws Exception
	{
		...
		ResourceData rr = cmgr.lookup(Resolver.QTYPE_A, host1);
		org.junit.Assert.assertNull(rr);
		org.junit.Assert.assertEquals(2, cache.size());
		rr = cmgr.lookup(Resolver.QTYPE_A, host2);
		org.junit.Assert.assertEquals(Resolver.QTYPE_A, rr.rrType());
		rr = cmgr.lookup(Resolver.QTYPE_A, host3);
		...
		rr = cmgr.lookup(Resolver.QTYPE_A, host8);
		org.junit.Assert.assertEquals(Resolver.QTYPE_A, rr.rrType());
		...
		Answer ans = new Answer().set(Answer.STATUS.OK, Resolver.QTYPE_A, host99);
		Answer.STATUS result = cmgr.storeResult(ans);
		org.junit.Assert.assertEquals(Answer.STATUS.NODOMAIN, result);
		org.junit.Assert.assertEquals(0, ans.rrdata.size());
		org.junit.Assert.assertEquals(6, cache.size());
		rr = cmgr.lookup(Resolver.QTYPE_A, host99);
		org.junit.Assert.assertEquals(6, cache.size());
		...
		org.junit.Assert.assertEquals(rr.toString(), Packet.QCLASS_INET, rr.rrClass());
		...
		ans = new Answer().set(Answer.STATUS.NODOMAIN, Resolver.QTYPE_A, host99b);
		rr = ResourceData.createNegativeRR(Resolver.QTYPE_A, ttl2);
		ans.rrdata.add(rr);
		...
		try {
			cmgr.lookup(Resolver.QTYPE_NS, host1);
			org.junit.Assert.fail("Did not trap invalid type-NS singular lookup");
		} catch (UnsupportedOperationException ex) {}
		org.junit.Assert.assertEquals(7, cache.size());

		ans = new Answer().set(Answer.STATUS.NODOMAIN, Resolver.QTYPE_ALL, new ByteChars("anyname"));
		try {
			cmgr.storeResult(ans);
			org.junit.Assert.fail("Did not trap invalid negative store qtype");
		} catch (IllegalArgumentException ex) {}
		ans = new Answer().set(Answer.STATUS.OK, Resolver.QTYPE_ALL, new ByteChars("anyname"));
		ans.rrdata.add(rr);
		try {
			cmgr.storeResult(ans);
	...
	public void testCacheList() throws Exception
	{
		...
		java.util.ArrayList<ResourceData> qrylst = cmgr.lookupList(Resolver.QTYPE_NS, ns1);
		...
		DynLoader.setField(dsptch, "systime_msecs", exptim + 1);
		qrylst = cmgr.lookupList(Resolver.QTYPE_NS, ns1);
		org.junit.Assert.assertNull(qrylst);
		org.junit.Assert.assertEquals(2, cache.size());

		qrylst = cmgr.lookupList(Resolver.QTYPE_NS, ns2);
		...
		qrylst = cmgr.lookupList(Resolver.QTYPE_NS, ns8);
		org.junit.Assert.assertEquals(1, qrylst.size());
		org.junit.Assert.assertEquals(5, cache.size());

		ByteChars ns99 = new ByteChars("host99.dom.org");
		Answer ans = new Answer().set(Answer.STATUS.OK, Resolver.QTYPE_NS, ns99);
		Answer.STATUS result = cmgr.storeResult(ans);
		org.junit.Assert.assertEquals(Answer.STATUS.NODOMAIN, result);
		org.junit.Assert.assertEquals(6, cache.size());
		qrylst = cmgr.lookupList(Resolver.QTYPE_NS, ns99);
		org.junit.Assert.assertEquals(6, cache.size());
		org.junit.Assert.assertEquals(1, qrylst.size());
		...
		try {
			cmgr.lookupList(Resolver.QTYPE_A, ns8);
	...
	public void testCacheIP() throws Exception
	{
		...
		DynLoader.setField(dsptch, "systime_msecs", dsptch.getSystemTime() + ttl1 + 1);
		ResourceData rr = cmgr.lookup(Resolver.QTYPE_PTR, ip1);
		org.junit.Assert.assertNull(rr);
		org.junit.Assert.assertEquals(2, cache.size());
		rr = cmgr.lookup(Resolver.QTYPE_PTR, ip2);
		org.junit.Assert.assertEquals(Resolver.QTYPE_PTR, rr.rrType());
		rr = cmgr.lookup(Resolver.QTYPE_PTR, ip3);
		org.junit.Assert.assertTrue(rr.toString(), rr.isNegative());
		org.junit.Assert.assertEquals(2, cache.size());
		...
		org.junit.Assert.assertEquals(5, cache.size()); //pruned back to lowater and then added host8
		rr = cmgr.lookup(Resolver.QTYPE_PTR, 108);
		org.junit.Assert.assertEquals(Resolver.QTYPE_PTR, rr.rrType());
		org.junit.Assert.assertTrue(rr.getName() == host108);
		org.junit.Assert.assertEquals(108, rr.getIP());
		org.junit.Assert.assertEquals(5, cache.size());

		rr = cmgr.lookup(Resolver.QTYPE_PTR, ip3+1);
		org.junit.Assert.assertNull(rr);

		Answer ans = new Answer().set(Answer.STATUS.OK, Resolver.QTYPE_PTR, 999);
		Answer.STATUS result = cmgr.storeResult(ans);
		org.junit.Assert.assertEquals(Answer.STATUS.NODOMAIN, result);
		org.junit.Assert.assertEquals(6, cache.size());
		rr = cmgr.lookup(Resolver.QTYPE_PTR, 999);
		org.junit.Assert.assertEquals(6, cache.size());
		org.junit.Assert.assertTrue(rr.toString(), rr.isNegative());
		org.junit.Assert.assertNull(rr.toString(), rr.getName());
		org.junit.Assert.assertEquals(rr.toString(), 0, rr.getIP());
		...
		try {
			cmgr.lookup(Resolver.QTYPE_A, 0);
	...
	private void createManager(XmlConfig dnscfg)
		throws com.grey.base.GreyException, java.io.IOException, javax.naming.NamingException
	{
		...
		Config config = new Config(dnscfg, logger);
	...
	{
		Answer.STATUS result = (ttl < 0 ? Answer.STATUS.NODOMAIN : Answer.STATUS.OK);
		long expiry = dsptch.getSystemTime() + Math.abs(ttl);
		Answer ans = new Answer().set(result, Resolver.QTYPE_A, hname);
		ResourceData rr = null;
		if (result == Answer.STATUS.OK) {
			rr = new ResourceData.RR_A(ans.qname, ip, expiry);
			ans.rrdata.add(rr);
		}
		Answer.STATUS result2 = cmgr.storeResult(ans);
		org.junit.Assert.assertEquals(result, result2);
		ans.rrdata.clear(); //to make sure cache storage is independent of this
		ResourceData rr2 = cmgr.lookup(Resolver.QTYPE_A, hname);
		org.junit.Assert.assertEquals(rr2.toString(), Packet.QCLASS_INET, rr2.rrClass());
		if (result == Answer.STATUS.OK) {
			org.junit.Assert.assertTrue(rr2 == rr);
			org.junit.Assert.assertEquals(rr2.toString(), expiry, rr2.getExpiry());
			org.junit.Assert.assertEquals(rr2.toString(), Resolver.QTYPE_A, rr2.rrType());
		...
		rr2 = cmgr.lookup(Resolver.QTYPE_A, new ByteChars(hname).append('x'));
		org.junit.Assert.assertNull(rr2);
	}

	private void storeNameServers(ByteChars domnam, java.util.ArrayList<ResourceData.RR_NS> lst)
	{
		Answer.STATUS result = (lst == null ? Answer.STATUS.NODOMAIN : Answer.STATUS.OK);
		java.util.ArrayList<ResourceData> lst_sav = null;
		Answer ans = new Answer().set(result, Resolver.QTYPE_NS, domnam);
		if (result == Answer.STATUS.OK) {
		...
		org.junit.Assert.assertEquals(result, result2);
		ans.rrdata.clear(); //to make sure cache storage is independent of this
		java.util.ArrayList<ResourceData> qlst = cmgr.lookupList(Resolver.QTYPE_NS, domnam);
		org.junit.Assert.assertEquals(result == Answer.STATUS.NODOMAIN ? 1 : lst_sav.size(), qlst.size());
		for (int idx = 0; idx != qlst.size(); idx++) {
			ResourceData rr = qlst.get(idx);
			org.junit.Assert.assertEquals(rr.toString(), Packet.QCLASS_INET, rr.rrClass());
			if (result == Answer.STATUS.NODOMAIN) {
				org.junit.Assert.assertTrue(rr.toString(), rr.isNegative());
			} else {
				org.junit.Assert.assertEquals(rr.toString(), Resolver.QTYPE_NS, rr.rrType());
				org.junit.Assert.assertTrue(rr.toString(), rr == lst_sav.get(idx));
			}
		}
		qlst = cmgr.lookupList(Resolver.QTYPE_NS, new ByteChars(domnam).append('x'));
		org.junit.Assert.assertNull(qlst);
	}

	private void storeIP(int ip, ByteChars hname, long ttl)
	{
		Answer.STATUS result = (hname == null ? Answer.STATUS.NODOMAIN : Answer.STATUS.OK);
		long expiry = dsptch.getSystemTime() + ttl;
		Answer ans = new Answer().set(result, Resolver.QTYPE_PTR, ip);
		ResourceData rr = null;
		if (result == Answer.STATUS.OK) {
		...
		org.junit.Assert.assertEquals(result, result2);
		ans.rrdata.clear(); //to make sure cache storage is independent of this
		ResourceData rr2 = cmgr.lookup(Resolver.QTYPE_PTR, ip);
		org.junit.Assert.assertEquals(rr2.toString(), Packet.QCLASS_INET, rr2.rrClass());
		if (result == Answer.STATUS.OK) {
			org.junit.Assert.assertTrue(rr2 == rr);
			org.junit.Assert.assertEquals(rr2.toString(), expiry, rr2.getExpiry());
			org.junit.Assert.assertEquals(rr2.toString(), Resolver.QTYPE_PTR, rr2.rrType());
			org.junit.Assert.assertTrue(rr2.toString(), rr2.getName() == hname);
			org.junit.Assert.assertEquals(rr2.toString(), ip, rr2.getIP());
		} else {
			org.junit.Assert.assertTrue(rr2.toString(), rr2.isNegative());
