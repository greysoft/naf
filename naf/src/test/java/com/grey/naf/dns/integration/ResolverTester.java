...
public class ResolverTester
{
	...
	public static void beforeClass() throws com.grey.base.GreyException, java.io.IOException
	{
		if (!USE_REAL_DNS) {
			mockserver = new TestServerDNS();
			mockserver.start();
		}
	}
	...
	protected void assertAnswer(Answer.STATUS exp_sts, byte exp_qtype, int exp_ip, Answer answer)
	{
		org.junit.Assert.assertNotNull(answer);
		logger.info("UTEST: Answer for "+Resolver.getQTYPE(exp_qtype)+"="+answer.qname+": "+answer);
		int rrsize = 1;
		if (exp_qtype == Resolver.QTYPE_NS || exp_qtype == Resolver.QTYPE_SRV) {
			rrsize = -2;
		} else if (exp_qtype == Resolver.QTYPE_MX) {
			rrsize = -1;
		}
		...
		if (answer.result == Answer.STATUS.OK && exp_qtype == Resolver.QTYPE_A) {
	...
	protected void assertAnswer(Answer.STATUS exp_sts, byte exp_qtype, CharSequence exp_domnam, Answer answer)
	{
		org.junit.Assert.assertNotNull(answer);
		logger.info("UTEST: Answer for IP/"+Resolver.getQTYPE(exp_qtype)+"="+IP.displayDottedIP(answer.qip)+": "+answer);
	...
	private void assertAnswer(Answer answer, Answer.STATUS exp_sts, byte exp_qtype, int exp_rrsize)
	{
		org.junit.Assert.assertEquals(exp_sts, answer.result);
		org.junit.Assert.assertEquals(exp_qtype, answer.qtype);
		if (answer.result == Answer.STATUS.NODOMAIN) rspcnt_nodom++;
		if (answer.result == Answer.STATUS.TIMEOUT) rspcnt_tmt++;

		if (exp_sts != Answer.STATUS.OK) exp_rrsize = 0;
		if (exp_rrsize < 0) {
		...
		for (int idx = 0; idx != answer.size(); idx++) {
			...
			if (answer.result == Answer.STATUS.OK) {
				if (exp_qtype == Resolver.QTYPE_SOA || exp_qtype == Resolver.QTYPE_TXT || exp_qtype == Resolver.QTYPE_AAAA) {
		...
		if (is_mockserver && answer.result == Answer.STATUS.OK) {
	...
	protected static void validateFinalState(Resolver r)
	{
	...
	protected static void validatePendingRequests(Resolver r, int exp_reqs, int exp_callers)
	{
		Object rs = getResolverService(r);
	...
	protected static Object getResolverService(Resolver r)
	{
