...
class TestServerDNS
	implements com.grey.naf.dns.server.Server.DNSQuestionResolver
{
	...
	private final com.grey.naf.dns.server.Server srvr;
	...
	public TestServerDNS() throws com.grey.base.GreyException, java.io.IOException {
		populateAnswers();
		...
		Dispatcher dsptch = Dispatcher.create(def, null, null);
		srvr = new com.grey.naf.dns.server.Server(this, dsptch, "127.0.0.1", 0);
		if (srvr.getLocalPort() == Packet.INETPORT) throw new IllegalStateException("DNS server not on ephemeral port");
	...
	{
		unused_answers.remove(Resolver.getQTYPE(qtype)+"="+qn);
		if (qtype == Resolver.QTYPE_A && StringOps.sameSeq(TMTDOMAIN, qn)) {
			if (tmtdomain_qrycnt++ % 2 == 0) {
				...
				byte[] buf = new byte[Packet.PKTHDRSIZ-1];
		...
		if (answer == null) {
			srvr.sendResponse(qid, qtype, qn, Packet.RCODE_NXDOM, true, recursion_desired, null, null, null,
					remote_addr, cbparam);
		} else {
			ResourceData[] auth = (answer.length > 1 ? answer[1] : null);
			ResourceData[] info = (answer.length > 2 ? answer[2] : null);
			boolean truncated = srvr.sendResponse(qid, qtype, qn, Packet.RCODE_OK, true, recursion_desired,
	...
	private void store(int rrtype, String name, ResourceData[][] data) {
		...
		map.put(name, data);
		unused_answers.add(Resolver.getQTYPE(rrtype)+"="+name);
	}

	private void storeA(String name, ResourceData[][] data) {store(Resolver.QTYPE_A, name, data);}
	private void storeNS(String name, ResourceData[][] data) {store(Resolver.QTYPE_NS, name, data);}
	private void storeSOA(String name, ResourceData[][] data) {store(Resolver.QTYPE_SOA, name, data);}
	private void storeMX(String name, ResourceData[][] data) {store(Resolver.QTYPE_MX, name, data);}
	private void storeSRV(String name, ResourceData[][] data) {store(Resolver.QTYPE_SRV, name, data);}
	private void storeTXT(String name, ResourceData[][] data) {store(Resolver.QTYPE_TXT, name, data);}
	private void storeAAAA(String name, ResourceData[][] data) {store(Resolver.QTYPE_AAAA, name, data);}
	...
	private void storePTR(String dotted_ip, ResourceData[][] data) {
		...
		store(Resolver.QTYPE_PTR, qname, data);
	}
