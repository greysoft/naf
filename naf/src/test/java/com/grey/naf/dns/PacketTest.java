...
public class PacketTest
{
	...
	private void verifyQuestion(boolean is_tcp, Packet pkt, Packet pkt2) {
		verifyQuestion(false, is_tcp, pkt, pkt2);
		verifyQuestion(true, is_tcp, pkt, pkt2);
	}

	private void verifyQuestion(boolean use_compression, boolean is_tcp, Packet pkt, Packet pkt2) {
		int qid = Short.MAX_VALUE;
		byte qtype = Resolver.QTYPE_A;
		...
		org.junit.Assert.assertEquals(Packet.QCLASS_INET, cb.cb_qclass);
		...
		org.junit.Assert.assertEquals(Packet.PKTHDRSIZ + boff, off);
		org.junit.Assert.assertEquals(1, pkt2.getQuestionCount());
		off = pkt2.skipSection(off, Packet.SECT_QUESTIONS, pkt2.getQuestionCount());
		org.junit.Assert.assertEquals(barr.length, off);
		org.junit.Assert.assertEquals(1, pkt2.getQuestionCount());
	...
	private void verifyCompressedNames(boolean is_tcp, boolean directbufs) throws Exception {
		java.lang.reflect.Method meth_encode = Packet.class.getDeclaredMethod("encodeName", int.class, ByteChars.class);
		meth_encode.setAccessible(true);
		java.lang.reflect.Method meth_decode = Packet.class.getDeclaredMethod("decodeName", int.class, ByteChars.class);
		...
		Packet pkt = new Packet(200, directbufs, 0); //more than big enough
		byte filler = 125;
	...
	private void verifyHeader(boolean is_tcp, boolean directbufs) {
		int pktsiz = Packet.PKTHDRSIZ;
		if (is_tcp) pktsiz += Packet.TCPMSGLENSIZ;
		Packet pkt = new Packet(pktsiz+10, directbufs, 5);
		...
		org.junit.Assert.assertEquals(0, niobuf.position());
		int tcp_incr = (is_tcp ? Packet.TCPMSGLENSIZ : 0);
		int boff = 1; //add an extra unused byte at both ends of array
		byte[] barr = new byte[pktsiz+boff+1];
		niobuf.get(barr, boff, pktsiz);
		if (is_tcp) org.junit.Assert.assertEquals(pktsiz - Packet.TCPMSGLENSIZ, ByteOps.decodeInt(barr, boff, Packet.TCPMSGLENSIZ));
	...
	private void verifyBlankHeader(Packet pkt) {
	...
	private static class ParseQuestionHandler implements Packet.MessageCallback {
		public int qcallcnt;
