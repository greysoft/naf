	...
	implements com.grey.naf.EntityReaper
{
		...
		extends CM_Stream
	{
		...
		public TestMonitor(Dispatcher d, java.nio.channels.SelectableChannel c, boolean takeOwnership,
			...
			org.junit.Assert.assertTrue(getChannel() == iochan);
			org.junit.Assert.assertFalse(iochan.isRegistered());
		}
	}
	...
	private static class CMR extends TestMonitor
	{
		...
		public CMR(Dispatcher d, java.nio.channels.SelectableChannel c, com.grey.naf.BufferSpec bufspec, int sends,
					CMW cmw, ChannelMonitorTest harness) throws com.grey.base.FaultException, java.io.IOException {
			super(d, c, true, bufspec, null, harness);
			numsends = sends;
			writer = cmw;
			org.junit.Assert.assertNull(chanwriter);
			chanreader.receive(0);
			org.junit.Assert.assertTrue(iochan.isRegistered());
		}
		...
		{
			for (int idx = 0; idx != rcvdata.ar_len; idx++) {
				org.junit.Assert.assertEquals(expectval++, rcvdata.ar_buf[rcvdata.ar_off+idx]);
			}
			...
			if (sendcnt == numsends && rcvbytes == writer.xmtbytes) {
				...
				java.nio.channels.SelectableChannel chan = iochan;
				org.junit.Assert.assertTrue(isConnected());
	...
	private static class CMW extends TestMonitor
	{
		...
		public CMW(Dispatcher d, java.nio.channels.SelectableChannel w, com.grey.naf.BufferSpec bufspec, ChannelMonitorTest runner)
			...
			pipe = w;
			org.junit.Assert.assertNull(chanreader);
		}
		...
		public void write(byte[] buf, int off, int len) throws java.io.IOException {
			...
			chanwriter.transmit(buf, off, len);
			xmtbytes += len;
		}
	...
	private void launch(boolean directbufs) throws com.grey.base.GreyException, java.io.IOException
	{
		...
		for (int idx = 0; idx != receivers.length; idx++) {
			...
			} while (!cmw.chanwriter.isBlocked());
		...
		for (int idx = 0; idx != receivers.length; idx++) {
			...
			for (int idx2 = 0; idx2 != cmpair.length; idx2++) {
				...
				org.junit.Assert.assertNull(cm.iochan);
