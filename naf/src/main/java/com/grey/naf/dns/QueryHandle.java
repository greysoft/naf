...
final class QueryHandle
	implements Resolver.Client, Packet.MessageCallback, com.grey.naf.reactor.Timer.Handler
{
	...
	private static final com.grey.base.utils.ByteChars ARPADOM0 = new com.grey.base.utils.ByteChars(IP.displayArpaDomain(0));

	public final ResolverService rslvr;
	private final java.util.ArrayList<WrapperRR> rrdata = new java.util.ArrayList<WrapperRR>(); //answer RRs
	...
	public com.grey.base.utils.ByteChars qname; //overall question for this request
	...
	@Override
	public long getSystemTime() {return rslvr.dsptch.getSystemTime();}
	...
	public QueryHandle init(byte qt, com.grey.base.utils.ByteChars qn)
	{
		qname = new com.grey.base.utils.ByteChars(qn);
		if (qname.charAt(qname.ar_len - 1) == Resolver.DOMDLM) qname.ar_len--;
		qip = 0;
		return init(qt);
	}
	...
	public Answer.STATUS issueQuery(java.net.InetSocketAddress srvaddr, boolean sticky_ip, Packet pkt)
	{
		pkt.resetEncoder(isTCP(), false); //we only ever encode 1 question, so avoid compression overhead
		pkt.hdr_qid = qid;
		pkt.hdr_qcnt = 1;
		if (rslvr.config.recursive) pkt.setRecursionDesired();
		...
		if (rslvr.logger.isActive(Config.DEBUGLVL)) {
			rslvr.logger.log(Config.DEBUGLVL, "DNS-Resolver sending "+(isTCP()?"TCP":"UDP")
					+" query="+qid+"/"+Resolver.getQTYPE(qtype)+"/"+qname+" to "+srvaddr
					+" - size="+pktlen);
		}
		Answer.STATUS result = send(niobuf, srvaddr);
	...
	public Answer.STATUS endRequest(Answer.STATUS result)
	{
		if (result == Answer.STATUS.OK && rrdata.size() == 0 && isFlagSet(F_NOTNXDOM)) result = Answer.STATUS.ERROR;
		Answer answer = setAnswer(result, null);

		if (result == Answer.STATUS.OK || result == Answer.STATUS.NODOMAIN) {
			for (int idx = 0; idx != rrdata.size(); idx++) {
				answer.rrdata.add(rrdata.get(idx).rr);
			}
			result = rslvr.cachemgr.storeResult(answer);
		}
		...
		rslvr.anstore.store(answer);
		rslvr.requestCompleted(this);
		return result;
	}

	public Answer endRequest(Answer answer)
	{
		clearAnswer();
	...
	public Answer repeatQuery(int server_ip)
	{
		clearFlag(F_HAVERSP);
		return rslvr.repeatQuery(this, server_ip);
	}

	private Answer.STATUS send(java.nio.ByteBuffer niobuf, java.net.InetSocketAddress srvaddr)
	{
		try {
			if (tcpconn != null) {
				tcpconn.send(niobuf);
			} else {
				EndPointUDP sender = rslvr.xmtmgr.nextSendPort();
				sender.send(niobuf, srvaddr);
			}
		} catch (Throwable ex) {
			Logger.LEVEL lvl = (ex instanceof RuntimeException ? Logger.LEVEL.ERR : Logger.LEVEL.TRC);
			if (rslvr.logger.isActive(lvl)) {
				rslvr.logger.log(lvl, ex, ex instanceof RuntimeException,
						"DNS-Resolver send failed on "+(isTCP()?"TCP":"UDP")+" for "+Resolver.getQTYPE(qtype)+"/"+qname
	...
	public Answer.STATUS switchTCP(java.net.InetSocketAddress nsaddr)
	{
		setFlag(F_TCP);
		tcpconn = rslvr.xmtmgr.allocateTCP();
		try {
			tcpconn.connect(this, nsaddr); //TCP should time itself out long before retrytmt_tcp expires
		} catch (Throwable ex) {
			Logger.LEVEL lvl = (ex instanceof RuntimeException ? Logger.LEVEL.ERR : Logger.LEVEL.TRC2);
			if (rslvr.logger.isActive(lvl)) {
				rslvr.logger.log(lvl, ex, lvl==Logger.LEVEL.ERR, "DNS-Resolver connect failed to "+nsaddr+" for "+Resolver.getQTYPE(qtype)+"/"+qname);
			}
			return endRequest(Answer.STATUS.TIMEOUT); //treat in same way as unsuccessful connected()
		}
		return Answer.STATUS.OK;
	}
	...
	public void handleResponse(Packet pkt, int off, java.net.InetSocketAddress srvaddr, int msglen)
	{
		// our response has been received - if any sub-queries take too long, well they have their own timers
		if (rslvr.logger.isActive(Config.DEBUGLVL)) {
			rslvr.logger.log(Config.DEBUGLVL, "DNS-Resolver received "+(isTCP()?"TCP":"UDP")+" response="+pkt.hdr_qid+"/"+Resolver.getQTYPE(qtype)+"/"+qname
		...
		if (pkt.isTruncated()) {
			...
			if (isTCP()) {
				...
				endRequest(Answer.STATUS.ERROR);
				return;
			}
			rslvr.stats_trunc++;
			Answer.STATUS sts = switchTCP(srvaddr);
			if (sts != Answer.STATUS.OK) endRequest(sts);
			return;
		}
		...
		boolean parse_auth = (qtype == Resolver.QTYPE_NS && !rslvr.config.recursive);
		...
		if (!parse_auth && pkt.hdr_anscnt == 0 && pkt.hdr_authcnt != 0) {
			...
			if (isFlagSet(F_AUTHREDIRECT)) {
				...
				endRequest(Answer.STATUS.BADRESPONSE);
				return;
			}
		...
		off = pkt.parseSection(off, qtype, Packet.SECT_ANSWERS, pkt.hdr_anscnt, srvaddr, this);
		...
		if (parse_auth && off != -1) {
			...
			off = pkt.parseSection(off, qtype, Packet.SECT_AUTH, pkt.hdr_authcnt, srvaddr, this);
			parsed_auth = true;
		}
		if (unresolved_cnt != 0 && pkt.hdr_infocnt != 0 && off != -1) {
			...
			if (!parsed_auth) off = pkt.skipSection(off, Packet.SECT_AUTH, pkt.hdr_authcnt);
			if (off != -1) off = pkt.parseSection(off, qtype, Packet.SECT_INFO, pkt.hdr_infocnt, srvaddr, this);
		}
		...
		if (authredirect_hack && rrdata.size() != 0) {
			...
			if (qtype == Resolver.QTYPE_A && qname.equals(rrdata.get(0).rr.getName())) {
				endRequest(Answer.STATUS.BADRESPONSE); //endless redirection loop
				return;
			}
		}
		evaluateResponse();
	...
	@Override
	public boolean handleMessageRR(int pktqid, int sectiontype, int rrnum, int rrcnt, com.grey.base.utils.ByteChars rrname, ResourceData rsprr, java.net.InetSocketAddress srvaddr)
	{
		Logger.LEVEL trclvl = Logger.LEVEL.TRC3;
		if (rslvr.logger.isActive(trclvl)) {
			rslvr.logger.log(trclvl, "Loading RR="+Packet.getSectionType(sectiontype)+"/"+rrnum+"/"+rrcnt+"="+rrname+" - "+rsprr
		...
		case Resolver.QTYPE_A: {
			...
			if (qtype == Resolver.QTYPE_A && sectiontype == Packet.SECT_ANSWERS) {
			...
			if (!rslvr.config.recursive && sectiontype == Packet.SECT_INFO) {
				if (rslvr.config.cache_all_glue || rsprr.getName().endsWith(qname)) {
					ResourceData rr = new ResourceData.RR_A((ResourceData.RR_A)rsprr, qname);
					rslvr.cachemgr.storeHostAddress(rr.getName(), rr);
				}
			}
		...
		case Resolver.QTYPE_NS: {
			...
			if (qtype != Resolver.QTYPE_NS && !isFlagSet(F_AUTHREDIRECT)) break;
	...
	{
		com.grey.base.utils.ByteChars domnam = getResolvableName(rsprr);
		int ip = IP.convertDottedIP(domnam);
		boolean have_ip = IP.validDottedIP(domnam, ip);
		ResourceData rr_a = null;
		if (!have_ip) {
			rr_a = rslvr.cachemgr.lookup(Resolver.QTYPE_A, domnam);
	...
	private boolean evaluateResponse(Answer answerbuf)
	{
		...
		for (int idx = rrdata.size() - 1; idx != -1; idx--) {
			...
			int server_ip = 0;
			com.grey.base.utils.ByteChars domnam = getResolvableName(rr);

			if (!rslvr.config.recursive && qtype == Resolver.QTYPE_NS) {
				if (qname_ip == 0) qname_ip = getPartialAnswer();
				com.grey.base.utils.ByteChars dom = rslvr.getParentDomain(Resolver.QTYPE_A, domnam);
				if (qname.equals(dom)) {
					...
					ResourceData rr_a = rslvr.cachemgr.lookup(Resolver.QTYPE_A, domnam);
					if (rr_a != null) {
						//the associated type-A RR has been cached due to activity in other requests
						resolveIP(idx, rr_a);
						continue;
					}
					java.net.InetSocketAddress nsaddr = rslvr.getNameserver(Resolver.QTYPE_A, domnam);
					if (nsaddr != null) {
						if (nsaddr.getPort() == 0) {
							discardUnresolvedRR(idx);
							continue;
			...
			Answer answer = issueSubQuery(Resolver.QTYPE_A, domnam, server_ip, answerbuf);
			...
			if (answer == null) {
				...
				if (answer.result == Answer.STATUS.OK) {
					...
					if (answer.result == Answer.STATUS.DEADLOCK) {
						wrr.deadlocked = true;
					} else {
						discardUnresolvedRR(idx);
					}
	...
	{
		Answer buf = rslvr.anstore.extract();
		boolean completed = false;
		try {
			completed = evaluateResponse(buf);
		} finally {
			rslvr.anstore.store(buf);
		}

		if (completed) {
			endRequest(Answer.STATUS.OK);
		} else {
			if (qtype == Resolver.QTYPE_NS && !isFlagSet(F_PARTIALNOTIFIED) && getResolvedCount() != 0) {
	...
	public Answer issueSubQuery(byte type, com.grey.base.utils.ByteChars name, int ip, Answer answerbuf)
	{
		if (rslvr.logger.isActive(Config.DEBUGLVL)) {
			rslvr.logger.log(Config.DEBUGLVL, "DNS-Resolver issuing subquery="+Resolver.getQTYPE(type)+"/"+name
					+" on behalf of query="+qid+"/"+Resolver.getQTYPE(qtype)+"/"+qname
					+" - state=0x"+Integer.toHexString(flags)+"/"+subqry_open+"/"+unresolved_cnt+"/"+rrdata.size());
		}
		Answer answer = rslvr.resolve(type, name, this, null, 0, ip, answerbuf);
		if (answer == null) {
			subqry_open++;
		} else {
			if (rslvr.logger.isActive(Config.DEBUGLVL)) {
				rslvr.logger.log(Config.DEBUGLVL, "DNS-Resolver has sync answer for "+qid+"/"+Resolver.getQTYPE(qtype)+"/"+qname
						+" subquery - "+answer);
			}
		}
		return answer;
	...
	@Override
	public void dnsResolved(com.grey.naf.reactor.Dispatcher d, Answer answer, Object callerparam)
	{
		if (rslvr.logger.isActive(Config.DEBUGLVL)) {
			rslvr.logger.log(Config.DEBUGLVL, "DNS-Resolver has answer for "+Resolver.getQTYPE(qtype)+"/"+qname+" subquery - "
					+answer+" QH-state=0x"+Integer.toHexString(flags)+"/"+subqry_open+"/"+unresolved_cnt+"/"+rrdata.size()+"/"+rrdata);
		}
		subqry_open--;

		if (answer.qtype == Resolver.QTYPE_NS) {
			//this is the response to an NS query we needed before we could proceed
			int server_ip = 0;
			if (answer.result != Answer.STATUS.OK) {
				if (answer.result == Answer.STATUS.NODOMAIN) {
		...
		if (answer.result == Answer.STATUS.OK) {
			handleMessageRR(qid, Packet.SECT_SUBQUERY, 0, 0, answer.qname, answer.rrdata.get(0), null);
			...
			if (answer.result != Answer.STATUS.NODOMAIN) setFlag(F_NOTNXDOM);
		}
	...
	private void notifyPartialAnswer()
	{
		for (int idx = callers.size() - 1; idx != -1; idx--) {
			if (callers.get(idx) instanceof QueryHandle) {
				...
				if (qhcaller.qtype == Resolver.QTYPE_NS || qhcaller.qtype == Resolver.QTYPE_A) {
					callers.remove(idx);
	...
	public int getPartialAnswer()
	{
		...
		if (cnt == 0) return 0;
		int selected = rslvr.rndgen.nextInt(cnt);
		int pos = 0;
		for (int idx = 0; idx != rrdata.size(); idx++) {
			int ip = rrdata.get(idx).rr.getIP();
			if (ip != 0) {
		...
		throw new IllegalStateException("Query="+Resolver.getQTYPE(qtype)+"/"+qname+" failed to locate partialIP="
				+selected+"/"+cnt+"/"+rrdata.size()+" - "+rrdata);
	}

	private int getUnresolvedRR(com.grey.base.utils.ByteChars domnam)
	{
		for (int idx = 0; idx != rrdata.size(); idx++) {
			ResourceData rr = rrdata.get(idx).rr;
			com.grey.base.utils.ByteChars rr_domnam = getResolvableName(rr);
			if (rr.getIP() == 0 && rr_domnam.equals(domnam)) return idx;
		}
		return -1;
	}

	private com.grey.base.utils.ByteChars getResolvableName(ResourceData rr)
	{
		com.grey.base.utils.ByteChars domnam = rr.getName();
		if (rr.rrType() == Resolver.QTYPE_NS) {
			domnam = ((ResourceData.RR_NS)rr).getHostname();
		} else if (rr.rrType() == Resolver.QTYPE_MX) {
			domnam = ((ResourceData.RR_MX)rr).getRelay();
		} else if (rr.rrType() == Resolver.QTYPE_SRV) {
	...
	private void resolveIP(ResourceData rr_unresolved, ResourceData rr_a)
	{
		...
		long ttl = (rslvr.config.setminttl ? Math.min(ttl_unresolved, ttl_a) : Math.max(ttl_unresolved, ttl_a));
	...
	private void notifyCallers(Answer answer)
	{
		while (callers.size() != 0) {
			Resolver.Client caller = callers.remove(0);
			Object param = callerparams.remove(0);
			try {
				caller.dnsResolved(rslvr.dsptch, answer, param);
			} catch (Throwable ex) {
	...
	public int cancelExternalCallers(Answer.STATUS result)
	{
		Answer answer = setAnswer(result, null);
		for (int idx = callers.size() - 1; idx != -1; idx--) {
			if (callers.get(idx) instanceof QueryHandle) continue;
			Resolver.Client caller = callers.remove(idx);
	...
	public boolean isCaller(Resolver.Client caller)
	{
	...
	@Override
	public boolean handleMessageQuestion(int pktqid, int qnum, int qcnt, byte qt, byte qclass, com.grey.base.utils.ByteChars qn, java.net.InetSocketAddress srvaddr)
	{
		if (qnum != 0) return false; //we only send one question per query, so expect only one echoed back
		if (qclass != Packet.QCLASS_INET || qt != qtype || !qn.equals(qname)) return false;
		if (isTCP()) return true;
		int remote_ip = IP.convertIP(srvaddr.getAddress());
		if (remote_ip != ip_request) {
			StringBuilder sb = rslvr.sbtmp;
			sb.setLength(0);
			...
			sb.append(" - request=").append(Resolver.getQTYPE(qtype)).append('/').append(qname);
	...
	private void startTimer(int type)
	{
		long tmt;
		if (tcpconn == null) {
			tmt = rslvr.config.retrytmt + (rslvr.config.retrystep * retrycnt) + rslvr.rndgen.nextInt((int)rslvr.config.retrystep);
		} else {
			tmt = rslvr.config.retrytmt_tcp;
		}
		if (tmr == null) {
			tmr = rslvr.dsptch.setTimer(tmt, type, this);
		} else {
			tmr.reset(tmt);
		}
	}
	...
	{
		if (rslvr.logger.isActive(Config.DEBUGLVL)) {
			rslvr.logger.log(Config.DEBUGLVL, "DNS-Resolver timeout="+t.type+"/"+(retrycnt+1)+"/"+(rslvr.config.retrymax+1)
					+" on "+Resolver.getQTYPE(qtype)+"/"+qname);
		}
		tmr = null;
		rslvr.stats_tmt++;

		// one timeout is enough to fail a TCP query
		if (isTCP() || (retrycnt == rslvr.config.retrymax)) {
			endRequest(Answer.STATUS.TIMEOUT);
			return;
		}
		retrycnt++;

		if (t.type == TMR_DEADLOCK) {
			evaluateResponse();
		} else if (t.type == TMR_TIMEOUT) {
			int server_ip = (isFlagSet(F_IPSTICKY) ? ip_request : 0);
			repeatQuery(server_ip);
		} else {
			throw new IllegalStateException("DNS-Resolver: Missing case for timer-type="+t.type);
		}
	}
	...
	private Answer setAnswer(Answer.STATUS result, Answer answer)
	{
		if (answer == null) answer = rslvr.anstore.extract();
		if (qip != 0) {
	...
	private int getMaxRRs()
	{
		if (qtype == Resolver.QTYPE_NS) {
			return rslvr.config.ns_maxrr;
		} else if (qtype == Resolver.QTYPE_MX) {
			return rslvr.config.mx_maxrr;
		}
		return 0;
	}
}
