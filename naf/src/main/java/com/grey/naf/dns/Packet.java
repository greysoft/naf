...
{
	public interface MessageCallback extends com.grey.naf.reactor.Timer.TimeProvider
	{
	...
	private static final String[] sect_txt = DynLoader.generateSymbolNames(Packet.class, "SECT_", 5);
	...
	public Packet() {this(0);} //minttl=0 would suit users who never encode a Packet and don't parse non-Question sections
	public Packet(long minttl) {this(0, false, minttl);} //would suit users who never encode a Packet

	public Packet(int bsiz, boolean bdirect, long minttl)
	{
		minttl_rr = minttl;
	...
	private ResourceData parseRDATA(int off, byte qtype, int datalen, byte rrtype, byte clss, int raw_ttl, MessageCallback handler)
	{
		...
		if (rrtype == Resolver.QTYPE_SOA) {
			if (qtype != Resolver.QTYPE_SOA) return null;
			rrbuf = rr_soa;
		} else if (rrtype == Resolver.QTYPE_A) {
			rrbuf = rr_a;
		} else if (rrtype == Resolver.QTYPE_PTR) {
			rrbuf = rr_ptr;
		} else if (rrtype == Resolver.QTYPE_NS) {
			rrbuf = rr_ns;
		} else if (rrtype == Resolver.QTYPE_MX) {
			rrbuf = rr_mx;
		} else if (rrtype == Resolver.QTYPE_TXT) {
			rrbuf = rr_txt;
		} else if (rrtype == Resolver.QTYPE_SRV) {
			rrbuf = rr_srv;
		} else if (rrtype == Resolver.QTYPE_CNAME) {
			rrbuf = rr_cname;
		} else if (rrtype == Resolver.QTYPE_AAAA) {
			if (qtype != Resolver.QTYPE_AAAA) return null; //ignore Additional AAAAs
			rrbuf = rr_aaaa;
		} else {
		...
		if (rrtype != Resolver.QTYPE_PTR) rrbuf.setName(rr_name);
	...
	public int encodeQuestion(int off, byte qtype, com.grey.base.utils.ByteChars domnam)
	{
		...
		return encodeInt(off, Packet.QCLASS_INET, FLDSIZ_RRCLASS);
	}

	public int encodeSection(int off, int sectiontype, ResourceData[] rr, com.grey.naf.reactor.Timer.TimeProvider time)
	{
	...
	private int encodeRR(int off, ResourceData rr, com.grey.naf.reactor.Timer.TimeProvider time)
	{
		long systime = (time == null ? System.currentTimeMillis() : time.getSystemTime());
		int ttl = rr.getTTL(systime);
		if (ttl < 0) ttl = 0;
		if (rr.rrType() == Resolver.QTYPE_PTR) {
	...
	{
		if (domnam != null && domnam.ar_len != 0) { //beware, the root domain name has no labels
			int labelcnt = domnam.count(Resolver.DOMDLM) + 1;
			int off_label = 0;
			for (int loop = 0; loop != labelcnt; loop++) {
				...
				int lmt_label = (loop == labelcnt - 1 ? domnam.size() : domnam.indexOf(off_label, Resolver.DOMDLM));
				int len_label = lmt_label - off_label;
				pktbuf[off_pkt++] = (byte)len_label;
				System.arraycopy(domnam.ar_buf, domnam.ar_off + off_label, pktbuf, off_pkt, len_label);
				off_pkt += len_label;
	...
	private int decodeName(int off, com.grey.base.utils.ByteChars domnam)
	{
		while (pktbuf[off] != 0) {
			...
			if (domnam != null) {
				domnam.ensureCapacity(domnam.ar_len + len + 1);
