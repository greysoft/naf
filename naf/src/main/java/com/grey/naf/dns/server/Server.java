...
public final class Server
{
	...
	static final int PKTSIZ_UDP = SysProps.get("greynaf.dns.maxudp", Packet.UDPMAXMSG);
	static final int PKTSIZ_TCP = SysProps.get("greynaf.dns.maxtcp", 5*PKTSIZ_UDP);
	static final int UDPSOCKBUFSIZ = SysProps.get("greynaf.dns.sockbuf", Packet.UDPMAXMSG * 128);
	...
	static final com.grey.logging.Logger.LEVEL DEBUGLVL = com.grey.logging.Logger.LEVEL.TRC2;

	private final com.grey.naf.reactor.Dispatcher dsptch;
	private final Server.DNSQuestionResolver responder;
	...
	private final Packet dnspkt = new Packet(Math.max(PKTSIZ_TCP, PKTSIZ_UDP), DIRECTNIOBUFS, 0);
	...
	public com.grey.naf.reactor.Dispatcher getDispatcher() {return dsptch;}

	// Binds server to random ephemeral port, if port param is zero
	public Server(DNSQuestionResolver responder, com.grey.naf.reactor.Dispatcher dsptch, String iface, int port)
	...
	public Server(DNSQuestionResolver r, com.grey.naf.reactor.Dispatcher d, com.grey.base.config.XmlConfig cfg)
			throws com.grey.base.GreyException, java.io.IOException
	{
		if (d == null) d = com.grey.naf.reactor.Dispatcher.create("DNS-Server", 0, null);
		dsptch = d;
		responder = r;
		handlers = new Handlers(this);
		recursion_offered = responder.dnsRecursionAvailable();
		com.grey.base.config.XmlConfig lcfg = (cfg == null ? null : cfg.getSection("listener"));
		java.util.Map<String,Object> cfgdflts = new java.util.HashMap<String,Object>();
		cfgdflts.put(com.grey.naf.reactor.CM_Listener.CFGMAP_PORT, Packet.INETPORT);
		cfgdflts.put(CM_Listener.CFGMAP_FACTCLASS, TransportTCP.ServerFactory.class);
		listener_tcp = new ConcurrentListener("DNS-Server", d, this, null, lcfg, cfgdflts);
		transport_udp = new TransportUDP(d, this, listener_tcp.getIP(),listener_tcp.getPort());
		dsptch.logger.info("DNS-Server: directbufs="+DIRECTNIOBUFS+"; udpmax="+PKTSIZ_UDP+"; tcpmax="+PKTSIZ_TCP);
		if (IGNORE_QTRAIL) dsptch.logger.info("DNS-Server: Will ignore trailing bytes in incoming queries");
	}
	...
	void queryReceived(com.grey.base.utils.ArrayRef<byte[]> rcvdata, java.net.InetSocketAddress remote_addr,
			TransportTCP tcp) throws java.io.IOException
	{
		...
		dnspkt.resetDecoder(rcvdata.ar_buf, rcvdata.ar_off, rcvdata.ar_len);
		...
		if (off == -1 || dnspkt.getQuestionCount() == 0) {
			sendResponse(dnspkt.getQID(), handlers.qry_qtype, handlers.qry_qname, Packet.RCODE_BADFMT, is_auth, dnspkt.recursionDesired(),
					null, null, null, remote_addr, tcp);
			return;
		}
		if (dsptch.logger.isActive(Server.DEBUGLVL)) {
			dsptch.logger.log(Server.DEBUGLVL, "DNS-Server received "+(tcp==null?"UDP":"TCP")+" query="
					+dnspkt.getQID()+"/"+Resolver.getQTYPE(handlers.qry_qtype)+"/"+handlers.qry_qname
					+" from "+remote_addr+" - size="+rcvdata.ar_len+", recurse="+dnspkt.recursionDesired());
		}
		if (dnspkt.opcode() != Packet.OPCODE_QRY) {
			dsptch.logger.info("DNS-Server rejecting unsupported packet="+dnspkt.getQID()+" with opcode="+dnspkt.opcode()
				+" - rcode="+dnspkt.rcode()+"/rsp="+dnspkt.isResponse()
				+" - query="+Resolver.getQTYPE(handlers.qry_qtype)+"/"+handlers.qry_qname+" from "+remote_addr);
			sendResponse(dnspkt.getQID(), handlers.qry_qtype, handlers.qry_qname, Packet.RCODE_NOTIMPL, is_auth, dnspkt.recursionDesired(),
					null, null, null, remote_addr, tcp);
			return;
		}
		...
		if (dnspkt.getAnswerCount() + dnspkt.getAuthorityCount() != 0
			...
			dsptch.logger.info("DNS-Server rejecting invalid packet="+dnspkt.getQID()+"/"+dnspkt.getQuestionCount()
		...
		if (dnspkt.getInfoCount() != 0) off = dnspkt.skipSection(off, Packet.SECT_INFO, dnspkt.getInfoCount());

		if (!IGNORE_QTRAIL) {
			int excess = rcvdata.ar_off + rcvdata.ar_len - off;
			if (excess != 0) {
				dsptch.logger.info("DNS-Server rejecting packet="+dnspkt.getQID()+" due to excess bytes="+excess
					+" - query="+Resolver.getQTYPE(handlers.qry_qtype)+"/"+handlers.qry_qname+" from "+remote_addr);
				sendResponse(dnspkt.getQID(), handlers.qry_qtype, handlers.qry_qname, Packet.RCODE_BADFMT, is_auth, dnspkt.recursionDesired(),
						null, null, null, remote_addr, tcp);
				return;
		...
		try {
			...
			dsptch.logger.log(Logger.LEVEL.ERR, ex, true, "DNS-Server failed to handle DNS query="+dnspkt.getQID()
				+"/"+handlers.qry_qtype+"/"+handlers.qry_qname+" from "+remote_addr);
			if (dsptch.isActive()) {
				sendResponse(dnspkt.getQID(), handlers.qry_qtype, handlers.qry_qname, Packet.RCODE_SRVFAIL, is_auth, dnspkt.recursionDesired(),
						null, null, null, remote_addr, tcp);
			}
		}
	}
			...
			java.net.InetSocketAddress remote_addr, Object tcpconn) throws java.io.IOException
	{
		...
		dnspkt.setResponse();
		dnspkt.opcode(Packet.OPCODE_QRY);
		dnspkt.rcode(rcode);
		...
		int off_ans = off;
		if (ans != null) off = dnspkt.encodeSection(off, Packet.SECT_ANSWERS, ans, null);
		if (auth != null) off = dnspkt.encodeSection(off, Packet.SECT_AUTH, auth, null);
		if (info != null) off = dnspkt.encodeSection(off, Packet.SECT_INFO, info, null);
		...
		if (dsptch.logger.isActive(Server.DEBUGLVL)) {
			dsptch.logger.log(Server.DEBUGLVL, "DNS-Server sending "+(is_tcp?"TCP":"UDP")+" response="
					+dnspkt.getQID()+"/"+Resolver.getQTYPE(qtype)+"/"+qname
	...
	private static com.grey.base.config.XmlConfig makeConfig(String iface, int port) throws com.grey.base.ConfigException
	{
		String xml_iface = (iface == null ? "" : " interface=\""+iface+"\"");
		String xml = "<srvdns><listener port=\""+port+"\""+xml_iface+"/></srvdns>";
		return com.grey.base.config.XmlConfig.makeSection(xml, "srvdns"); //dummy root name that gets stripped off
	}

	
	// Move the override methods into an inner class, purely so as not to be externally visible
	private static final class Handlers
		implements com.grey.naf.dns.Packet.MessageCallback, com.grey.naf.EntityReaper
	{
		...
		Handlers(Server s) {srvr=s; dsptch=srvr.getDispatcher();}
		...
		public boolean handleMessageQuestion(int qid, int qnum, int qcnt, byte qt, byte qclass, ByteChars qn, java.net.InetSocketAddress remote_addr) {
			if (qclass != Packet.QCLASS_INET) {
				dsptch.logger.info("DNS-Server rejecting class="+qclass+" for query="+qt+"/"+qn+" from "+remote_addr);
				return false;
			}
			if (qnum != 0) {
				dsptch.logger.info("DNS-Server rejecting excess question #"+qnum+"/"+qcnt+" = "+qt+"/"+qn+" from "+remote_addr);
				return false;
			}
			qry_qtype = qt;
