...
final class CacheManager
{
	...
	public CacheManager(com.grey.naf.reactor.Dispatcher d, Config c) throws com.grey.base.ConfigException
	{
		dsptch = d;
		config = c;
	...
	{
		java.util.ArrayList<ResourceData> rrlst = lookupList(Resolver.QTYPE_NS, domnam);
	...
	{
		if (qtype != Resolver.QTYPE_PTR) throw new UnsupportedOperationException(LOGLBL+"lookup qtype="+qtype+" - "+qip);
	...
	public Answer.STATUS storeResult(Answer ans)
	{
		if (ans.result == Answer.STATUS.NODOMAIN || ans.rrdata.size() == 0) {
			ans.result = Answer.STATUS.NODOMAIN;
		...
		if (ans.result == Answer.STATUS.NODOMAIN) ans.rrdata.clear();
		return ans.result;
	}
	...
	private void storeSingleResult(String desc, Answer ans, HashedMap<ByteChars, ResourceData> cache, int lowater, int hiwater)
	{
	...
	private void storeListResult(String desc, Answer ans, HashedMap<ByteChars, java.util.ArrayList<ResourceData>> cache,
			int lowater, int hiwater)
	{
	...
	public void loadRootServers() throws com.grey.base.ConfigException
	{
		HashedMap<ByteChars, java.util.ArrayList<ResourceData>> newroots;
		try {
			newroots = config.loadRootServers(logger);
		} catch (Exception ex) {
			if (ns_roots.size()==0) throw new com.grey.base.ConfigException(ex, LOGLBL+"Failed to bootstrap root servers");
