	...
	implements Timer.Handler
{
	...
	public boolean unregister(Object flushable)
	{
		...
		try {
			...
			dsptch.logger.error("Failed to flush "+flushable+" on unregister - "+com.grey.base.GreyException.summary(ex));
		}
		return true;
	}
	...
	@Override
	public void timerIndication(Timer t, Dispatcher d)
	{
		tmr = null;
		flushAll();
	...
	public void flushAll()
	{
		...
		for (int idx = 0; idx != flushables.size(); idx++) {
			...
			try {
				...
				dsptch.logger.error("Failed to flush "+flushable+" - "+com.grey.base.GreyException.summary(ex));
