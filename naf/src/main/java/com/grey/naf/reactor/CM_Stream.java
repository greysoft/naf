...
{
	protected final IOExecReaderStream chanreader;
	protected final IOExecWriter chanwriter;
	SSLConnection sslconn;

	protected abstract void ioReceived(com.grey.base.utils.ArrayRef<byte[]> rcvdata)
	...
	protected final void registerConnectedChannel(java.nio.channels.SelectableChannel chan, boolean takeOwnership)
		throws java.io.IOException
	{
		registerChannel(chan, takeOwnership, true, true);
	}

	final void registerChannel(java.nio.channels.SelectableChannel chan, boolean takeOwnership, boolean isconn,
			boolean app_knows)
		throws java.io.IOException
	{
	...
	StringBuilder dumpChannelState(StringBuilder sb, String dlm)
	{
		...
		sb.append("<br/>Endpoint: ").append(usingSSL()?"SSL/":"").append(iochan);
		return sb;
	}
	...
	final void brokenPipe(LEVEL lvl, CharSequence discmsg, CharSequence errmsg, Throwable ex) throws BrokenPipeException
	{
		if (dsptch.logger.isActive(lvl)) {
			dsptch.logger.log(lvl, ex, false, getClass().getName()+"/E"+getCMID()+" "+errmsg
