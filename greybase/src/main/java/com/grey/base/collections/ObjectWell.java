	...
	extends ObjectPile<T>
{
	...
	{
		java.util.ArrayList<ObjectWell<?>> wells = new java.util.ArrayList<ObjectWell<?>>();

		public ShutdownHook() {
			Runtime.getRuntime().addShutdownHook(this);
		}

		public synchronized void add(ObjectWell<?> well) {
			wells.add(well);
		}
		@Override
		public void run() {
			System.out.println("\nObjectWell: Shutdown Thread=T"+Thread.currentThread().getId()+" - Wells="+wells.size());
			for (ObjectWell<?> well : wells) {
				if (well == null) continue;
				System.out.println(" - Well="+well.name+"/"+System.identityHashCode(well)+": alloc="
						+(well.population() - well.size())+", extant="+well.population()+"/"+well.maxPopulation());
			}
		}
	}
}